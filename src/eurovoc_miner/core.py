...
def get_docs(d, lang=None, days=1):
    ...
    Exceptions from the SPARQL fetch (timeouts, connection errors, malformed
    JSON) propagate to the caller. Previously they were caught here and turned
    into a silent empty-result, which caused callers to overwrite previously
    good parquets with empty ones during transient Cellar outages.
    """
